...
import type { SanasuppiloStats } from '../../../storage/sanasuppiloStats'

export interface SanasuppiloStatsModalProps {
  stats: SanasuppiloStats
  onClose: () => void
}

/**
 * Unlike Sanuri's StatsModal there's no per-length axis to break stats down
 * by -- every Sanasuppilo game draws from the same pool -- so this is just
 * an aggregate running total, reusing the same generic `stats.*` labels.
 */
export function SanasuppiloStatsModal({ stats, onClose }: SanasuppiloStatsModalProps) {
  const { t } = useI18n()
  ...
  const rows: [string, number][] = [
    ...
    [t('stats.won'), stats.won],
    [t('stats.streak'), stats.currentStreak],
    [t('stats.maxStreak'), stats.maxStreak],
  ]
