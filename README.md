...
The portal also hosts Sanuri Pro (the full dictionary), Sanasuppilo (a word-grouping puzzle) and
...
`src/data/sanajahti-words.json` is Sanajahti's own dictionary, generated by
`scripts/build-sanajahti-words.mjs` from the same Kotus list: every headword of four or more
letters made only of lowercase Finnish letters, of any length and any word class (so pronouns,
conjunctions and interjections count too; capitalised proper nouns and entries with hyphens or
digits do not). It is separate from the Sanuri lists, which stay limited to 4-7 letter nouns,
