...
/** Loads every scripts/data/seed-categories/*.json file into { [categoryId]: words[] }, keyed by filename (without extension). */
async function loadSeedCategories() {
  const fileNames = (await readdir(seedCategoriesDir)).filter((name) => name.endsWith('.json'))
